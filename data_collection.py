...
def add_record(user_scores, screen_name, type):
    if screen_name in user_scores.keys():
        ...
        user_scores.update({screen_name: {"likes": 0, "retweets": 0, "replies": 0}})
        return user_scores
...
def get_scores(screen_name, pages):
    ...
    for tweet in timeline:
        if (
            ...
            user_scores = add_record(
                user_scores, tweet.in_reply_to_screen_name, "replies"
            )
        if (
            ...
            user_scores = add_record(
                user_scores, tweet.retweeted_status.user.screen_name, "retweets"
            )
...
def select_users(user_scores):
    ...
    for user in user_scores:
        total = (
            (user_scores[user]["likes"] * 1)
            + (user_scores[user]["replies"] * 1.1)
            + (user_scores[user]["replies"] * 1.3)
        )
...
def combine_avatars(selected_users, avatars):
    ...
    for user in selected_users:
        try:  # remove invalid users such as deleted accounts
            temp_dict[user] = {"score": selected_users[user], "avatar": avatars[user]}
        except:
            pass
...
def define_layers(selected_users, layers_config):
    ...
    for config in layers_config:
        ...
        for user in selected_users:
            if (
                selected_users_list.index(user) >= starting_index
                and selected_users_list.index(user) < ending_index
            ):
                users.update({user: selected_users[user]})
            else:
                pass
        config.update({"users": users})
        users_in_circles.update(
            {f"circle{layers_config.index(config)}": [username for username in users]}
        )  # list comprehension
