...
def create_image(center_avatar_url, color, layers_config):
    ...
    bg.paste(
        center_avatar,
        ((bg_w - 160) // 2, (bg_h - 160) // 2),
        create_mask(center_avatar),
    )
    ...
    for layer in layers_config:
        ...
        for user in users:
            ...
            x = math.ceil(R * angle_x + 445) + (
                layers_config.index(layer) * 8
            )  # offset to address circles misalignment
            y = math.ceil(R * angle_y + 445) + (
                layers_config.index(layer) * 8
            )  # offset to address circles misalignment
